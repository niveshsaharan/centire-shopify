...
trait WebhookControllerTrait
{
    ...
    public function handle($type)
    {
        ...
        if (! $classPath) {
            \Log::error("Missing webhook job: {$classPath}");
    ...
    {
        $jobClassPath = '\\App\\Jobs\\' . str_replace('-', '', ucwords($type, '-')) . 'WebhookJob';

        if (class_exists($jobClassPath)) {
            return $jobClassPath;
        }

        $jobClassPath = '\\Centire\\ShopifyApp\\Jobs\\' . str_replace('-', '', ucwords($type, '-')) . 'WebhookJob';
